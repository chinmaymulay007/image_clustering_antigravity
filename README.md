...
### Core AI Engine
- **Local CLIP Embedding**: Uses OpenAI's CLIP (ViT-Base-Patch16) quantized for the browser to extract 512-dimensional semantic vectors from images.
...
1.  **Step 1: Scan & Embed** — Scans your selected folder for images (including subfolders), generates 512-dimensional CLIP embedding vectors, and extracts EXIF metadata (date taken, GPS coordinates). Previously computed embeddings are loaded from IndexedDB for instant resume.
2.  **Step 2: Dual Cluster Update** — Every *N* images (configurable batch size: 20, 40, or 100):
    - **Visual Clusters** are refreshed via K-Means with warm-start stability.
